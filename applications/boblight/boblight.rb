...
require REQUIREMENTS + "/../../lib/libambx"
...
if Ambx.connect
  if Ambx.open
    while true
      ...
      Ambx.write_all([ 0xA1, Lights::LEFT, 0x03, Float(r1) * 255, Float(g1) * 255, Float(b1) * 255 ])
      Ambx.write_all([ 0xA1, Lights::RIGHT, 0x03, Float(r2) * 255, Float(g2) * 255, Float(b2) * 255 ])
      Ambx.write_all([ 0xA1, Lights::WWLEFT, 0x03, Float(r3) * 255, Float(g3) * 255, Float(b3) * 255 ])
      Ambx.write_all([ 0xA1, Lights::WWRIGHT, 0x03, Float(r4) * 255, Float(g4) * 255, Float(b4) * 255 ])
      Ambx.write_all([ 0xA1, Lights::WWCENTER, 0x03, Float(r5) * 255, Float(g5) * 255, Float(b5) * 255 ])
    end
    Ambx.close
  else
